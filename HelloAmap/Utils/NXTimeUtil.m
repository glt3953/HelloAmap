...
+ (NSUInteger) nowZeroDayTime
{
    ...
    dateFormatter1.timeZone = [NSTimeZone timeZoneWithName:@"Europe/London"];
...
+ (NSString *) titleTime:(NSUInteger) timeInterval
{
    ...
    NSDateFormatter* dateFormatter=[[NSDateFormatter alloc] init];
    dateFormatter.dateFormat=[NSString stringWithFormat:@"yyyy%@MM%@dd%@", lang(@"Year"),lang(@"Moon"),lang(@"Day")];
    dateFormatter.dateFormat=[NSString stringWithFormat:@"yyyy%@MM%@dd%@",lang(@"Year"),lang(@"Moon"),lang(@"Day")];
    dateFormatter.timeZone = [NSTimeZone timeZoneWithName:@"beijing"];
...
+ (NSString *)friendlyTimeFromDateTime:(NSString *)time {
    ...
    if (tm_now.tm_year == tm_in.tm_year) {
        if (delta <= 0 ) {
            ...
            if (timeInterval <= 60) {
                ...
                format = [[NSString stringWithFormat:@"%@ ", lang(@"Today")] stringByAppendingString:timeFormat];
            }
        } else if (delta > 0 && delta <= SECONDS_PER_DAY) {
            //昨天
            format = [[NSString stringWithFormat:@"%@ ", lang(@"Yesterday")] stringByAppendingString:timeFormat];
        } else if (delta > SECONDS_PER_DAY) {
            //今年内
            format = [[NSString stringWithFormat:@"%@ ", lang(@"ThisYearFormat")] stringByAppendingString:timeFormat];
        }
    } else {
        //往年
        format = [[NSString stringWithFormat:@"%@ ", lang(@"PreviousYearsFormat")] stringByAppendingString:timeFormat];
    }
