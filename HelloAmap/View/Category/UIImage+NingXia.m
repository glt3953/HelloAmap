...
+ (UIImage *)nx_makeThumbnailFromImage:(NSData *)data
{
    ...
    CGSize sizeNew = [self nx_getThumbnailSizeFromSize:image.size];
...
+ (UIImage *)nx_makeImageFromText:(NSString *)text font:(UIFont *)font color:(UIColor *)color bgColor:(UIColor *)bgColor radius:(CGFloat)radius {
    ...
    [text drawAtPoint:CGPointMake(radius - size.width/2, radius - size.height/2) withFont:font];
...
- (UIImage *)nx_resizedImage:(CGSize)newSize interpolationQuality:(CGInterpolationQuality)quality {
    BOOL drawTransposed;
